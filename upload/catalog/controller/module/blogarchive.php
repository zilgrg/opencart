         ...
         foreach ($years as $year) {
            ...
            if ($months) {
               foreach ($months as $month) {
                  //=== Articles
                  $article_data = array();
                  $articles = $this->model_blog_article->getArchiveArticle($year['value'], $month['value'], 0, $data, $date);
                  ...
                  //=== Months
                  if ($setting['count']) {
                     $article_total = '[' . count($this->model_blog_article->getArchiveArticle($year['value'], $month['value'], 0, $data, $date)) . '] ';
                  } else {
                     $article_total = false;
                  }
            ...
            //=== Years
            if ($setting['count']) {
               $article_total = '[' . count($this->model_blog_article->getArchiveArticle($year['value'], 0, 1, $data, $date)) . '] ';
            } else {
               $article_total = false;
            }
