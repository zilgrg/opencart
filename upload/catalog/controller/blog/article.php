...
class ControllerBlogArticle extends Controller {
      ...
      $date = date('Y-m-d H:i:s');
		...
		if ($articleData) {
         ...
         $imageHeight = $blogSetting['articleFeatHeight'];
            ...
            $commentStatus = 1;
            $totalComments = $this->model_blog_article->getTotalCommentsByArticleId($this->request->get['article_id'], $date);
               if ($totalComments) {
			...
			} elseif ($blogSetting['commentStatus'] && $articleData['comment'] != '0') {
			...
			$this->data['artRelateds']			= $this->model_blog_article->getRelatedArticle($articleData['article_id'], $date);
	...
	public function comment() {
      ...
      $date = date('Y-m-d H:i:s');
		...
		$this->data['comment_nofollow_link'] = $blogSetting['commentNofollowLink'];
		

		if (isset($this->request->get['cmt_page'])) { $page = $this->request->get['cmt_page']; } else { $page = 1; }
		if ($blogSetting['commentLimit'] < 1) {
         $limit = 6;
      } else {
		...
		$results = $this->model_blog_article->getCommentsByArticleId($article_id, ($page - 1) * $limit, $limit, $blogSetting['commentOrder'], $date);
		...
		foreach ($results as $result) {
            ...
            $replyDataName = $this->model_blog_article->getReplyAdminName($replyData['comment_id']);
            if (isset($replyDataName['adminName'])) {
               $name = $replyDataName['adminName'];
            } else {
               $name = $replyData['name'];
            }
      ...
      $badgeColors	= unserialize($blogSetting['commentBadgeColor']);
		...
		$commentsTotal = $this->model_blog_article->getTotalCommentsByArticleId($article_id, $date);
	...
	public function write() {
      ...
      if ($this->registry->has('shortcodes')) { // Check if registry conatain shortcodes and strip user input
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && !isset($json['error'])) {
			...
			$articleComment = $this->model_blog_article->getArticle($this->request->get['article_id'], $date);
