...
<div id="shipping-new" style="display: <?php echo ($addresses ? 'none' : 'block'); ?>;">
  ...
  <?php } else { ?>
	<input type="text" style="display: none;" name="company" value="" class="large-field" />
  <?php } ?>
			...
			$.ajax({
				...
				success: function(json) {
					...
					<?php if ($this->config->get('quickcheckout_country_reload')) { ?>
						reloadShippingMethod($('#shipping-address select[name=\'country_id\']').val(), $('#shipping-address select[name=\'zone_id\']').val(), 0 , $('#shipping-address input[name=\'city\']').val(), $('#shipping-address input[name=\'postcode\']').val());
					<?php } ?>
				},
