...
class ModelBlogArticle extends Model {
   ...
   public function getArticles($data = array()) {
      ...
      if (!$article_data) {
         ...
         $sql .= " WHERE ad.language_id = '" . (int)$this->config->get('config_language_id') . "' AND a.status = '1' AND a.available <= '" . $data['date'] ."' AND a2s.store_id = '" . (int)$this->config->get('config_store_id') . "'"; 
         ...
         if (isset($data['exclude_category']) && is_array($data['exclude_category']) && !empty($data['exclude_category'])) {
            $implode_data = array();
				...
				$sql .= " AND a.article_id NOT IN (SELECT a2c.article_id FROM " . DB_PREFIX . "blog_article_to_category a2c WHERE " . implode(' OR ', $implode_data) . ")";
         }
         ...
         foreach ($query->rows as $result) {
            $article_data[$result['article_id']] = $this->getArticle($result['article_id'], $data['date']);
         }
   ...
   public function getRelatedArticleAtProduct($product_id, $date) {
      $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_product_related pr LEFT JOIN " . DB_PREFIX . "blog_article a ON (pr.article_id = a.article_id) LEFT JOIN " . DB_PREFIX . "blog_article_to_store a2s ON (a.article_id = a2s.article_id) WHERE pr.product_id = '" . (int)$product_id . "' AND a.status = '1' AND a.available <= '" . $date ."' AND a2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
      
      $article_data = array();
      
      foreach ($query->rows as $result) {
         $article_data[] = $this->getArticle($result['article_id'], $date);
      }
   ...
   public function addComment($article_id, $data) {
      $this->db->query("INSERT INTO " . DB_PREFIX . "blog_comment SET parent_id = '" . (int)$data['parent_id'] . "', customer_id = '" . (int)$this->customer->getId() . "', article_id = '" . (int)$article_id . "', name = '" . $this->db->escape($data['name']) . "', email = '" . $this->db->escape($data['email']) . "', website = '" . $this->db->escape($data['website']) . "', content = '" . $this->db->escape($data['content']) . "', status = '" . (int)$data['status'] . "', created = '" . $data['date'] ."'");
   }
   ...
   public function getReplyName() {
      $query = $this->db->query("SELECT a.name as replyName, a.user_id as replyStaff FROM " . DB_PREFIX . "blog_author a LEFT JOIN " . DB_PREFIX . "blog_comment c ON ( a.user_id = c.staff_id) WHERE c.status = '1'");
   ...
   public function getCommentsByArticleId($article_id, $start = 0, $limit = 100, $order = 'ASC', $date) {
      $query = $this->db->query("SELECT c.comment_id, c.parent_id, c.customer_id, c.name, c.email, c.website, c.content, c.staff_id, c.created FROM " . DB_PREFIX . "blog_comment c LEFT JOIN " . DB_PREFIX . "blog_article a ON (c.article_id = a.article_id) WHERE a.article_id = '" . (int)$article_id . "' AND c.status = '1' AND a.status = '1' AND c.parent_id = '0' AND a.available <= '" . $date ."' ORDER BY c.created " . $this->db->escape($order) . " LIMIT " . (int)$start . "," . (int)$limit);
      
      return $query->rows;
   }
   
   public function getReplyComments($comment_id, $order = 'ASC') {
      $query = $this->db->query("SELECT c.comment_id, c.parent_id, c.customer_id, c.name, c.email, c.website, c.content, c.staff_id, c.created FROM " . DB_PREFIX . "blog_comment c LEFT JOIN " . DB_PREFIX . "blog_article ba ON (c.article_id = ba.article_id) WHERE c.parent_id = '" . (int)$comment_id . "' AND c.status = '1' ORDER BY c.created " . $this->db->escape($order) . " LIMIT 0,   100");
      
      return $query->rows;
   }
   
   public function getReplyAdminName($comment_id) {
      $query = $this->db->query("SELECT a.name as adminName FROM " . DB_PREFIX . "blog_comment c LEFT JOIN " . DB_PREFIX . "blog_author a ON (c.staff_id = a.user_id) WHERE c.comment_id = '" . (int)$comment_id . "' AND c.status = '1'");
      if (isset($query->row)) {
         return $query->row;
      } else {
         return 0;
   ...
   public function getTotalCommentsByArticleId($article_id, $date) {
      $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "blog_comment c LEFT JOIN " . DB_PREFIX . "blog_article a ON (c.article_id = a.article_id) WHERE a.article_id = '" . (int)$article_id . "' AND c.parent_id = '0' AND c.status = '1' AND a.status = '1' AND a.available <= '" . $date ."'");
   ...
   public function getTags($limit = 20, $date) {
      $tag        = array();
      $total      = array();
      $tagsData   = array();
   ...
   public function getArchiveArticle($years, $group, $total = 0, $data = array(), $date) {
      $sql = "SELECT DISTINCT ";
      
      if ($total) { //assuming only year input use this
         $sql .= " a.article_id FROM " . DB_PREFIX . "blog_article a LEFT JOIN " . DB_PREFIX . "blog_article_to_category a2c ON (a.article_id = a2c.article_id) LEFT JOIN " . DB_PREFIX . "blog_article_to_store a2s ON (a.article_id = a2s.article_id) WHERE a.status = '1' AND a2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND YEAR(a.created) = " . $years . " AND a.available <= '" . $date ."'";
      } else {
         $sql .= " a.article_id FROM " . DB_PREFIX . "blog_article a LEFT JOIN " . DB_PREFIX . "blog_article_to_category a2c ON (a.article_id = a2c.article_id) LEFT JOIN " . DB_PREFIX . "blog_article_to_store a2s ON (a.article_id = a2s.article_id) WHERE a.status = '1' AND a2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND MONTH(a.created) = '" . $group . "' AND YEAR(a.created) = " . $years . " AND a.available <= '" . $date ."'";
      }
      ...
      if ($total) {
         ...
         $query = $this->db->query($sql);
         foreach ($query->rows as $result) {
            $article_data[$result['article_id']] = $this->getArticle($result['article_id'], $date);
         }
