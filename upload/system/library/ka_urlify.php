...
A part of this code was based on:
	...
	File    : URLify.php
	Version : jbroadway-urlify-e263633
...
class KaUrlify {
	 ...
	 */
	public static function filter ($text, $length = 90) {
		$text = self::downcode (trim($text));

		// remove all these words from the string before urlifying
		// $text = preg_replace ('/\b(' . join ('|', self::$remove_list) . ')\b/i', '', $text);
		
		// if downcode doesn't hit, the char will be stripped here
		$text = preg_replace ('/[^_-\w\s]/', '', $text);    // remove unneeded chars

		$text = trim(substr($text, 0, $length), '-_');      // trim to first $length chars
						
		// $text = preg_replace ('/^\s+|\s+$/', '', $text); // trim leading/trailing spaces
		$text = preg_replace ('/[-\s]+/', '-', $text);      // convert spaces to hyphens
		$text = strtolower ($text);                         // convert to lowercase
		
		if (preg_match('/[^-_a-z\d]/', $text, $matches)) {
			return false;
		}
