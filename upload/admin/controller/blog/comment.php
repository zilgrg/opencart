...
class ControllerBlogComment extends Controller {
	...
	public function reply() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
         $date = date('Y-m-d H:i');
			$this->model_blog_comment->replyComment($this->request->get['reply_id'], $this->request->post, $date);
            ...
            $result         = $this->model_blog_comment->getComment($this->request->get['reply_id']);
            ...
            if(isset($blogSetting['emailNotify']) && $blogSetting['emailNotify']) {
                $sendTo		= $result['email'];
                $subject	= sprintf($this->language->get('text_mail_subject'), $this->config->get('config_name'), $article['title']);
                
                $message  	= sprintf($this->language->get('text_mail_hello'), $result['name']) . "\n\n";
                $message  	.= $this->language->get('text_mail_greeting') . "\n\n";
                $message  	.= sprintf($this->language->get('text_mail_link'), HTTP_CATALOG . 'index.php?route=blog/article' . '&article_id=' . $result['article_id']) . "\n\n";
                $message 	.= sprintf($this->language->get('text_mail_message'), $this->request->post['content']) . "\n\n";
                $message	.= $this->language->get('text_mail_regards1') . "\n\n";
                $message	.= sprintf($this->language->get('text_mail_regards2'), $this->config->get('config_name')) . "\n\n";
	...
	private function getList() {
		...
		foreach ($results as $result) {
			...
			$adminName	 = $this->model_blog_comment->getReplyAdminName($result['comment_id']);
			...
			if (isset($adminName['staffId'])) {
				$filter_name = $adminName['staffId'];
			} else {
			...
			$this->data['comments'][] = array (
				...
				'filter_name'  => $this->url->link('blog/comment', 'token=' . $this->session->data['token'] . $url . '&filter_name=' . $filter_name, 'SSL'),
				'in_reply'		=> $parentName,
				'name'			=> $name,
				'email'			=> $result['email'],
      ...
      if (isset($this->request->get['filter_email'])) {
        ...
        $help_page      = rawurlencode('11469106#BMComments-CommentList');
	...
	private function getForm() {
      ...
      $authorName		= $this->model_blog_comment->getAuthorName($this->user->getId());
        ...
        $help_meta      = $this->language->get('help_meta_reply'); // @todo language file
        $help_page      = rawurlencode('11469106#BMComments-CommentReply');
        $this->data['online_help_url']   = $this->url->link('blog/help', 'meta=' . $help_meta . '&page=' . $help_page . '&token=' . $this->session->data['token'], 'SSL');
