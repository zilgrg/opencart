...
class ControllerBlogArticle extends Controller {
	...
	public function insert() {
      ...
      if (isset($this->request->get['page'])) {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
         $date = date('Y-m-d H:i');
			$this->model_blog_article->addArticle($this->request->post, $date);
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
         $date = date('Y-m-d H:i');
			$this->model_blog_article->editArticle($this->request->get['article_id'], $this->request->post, $date);
	...
	private function getForm() {
      ...
      $this->load->model('setting/setting');
		...
		$this->data['autoArtKeyword'] = $blogSetting['autoArtKeyword'];
		...
		if (isset($article_info)) {
			$this->data['modified'] = date('Y-m-d H:i', strtotime($article_info['modified']));
		} else {
			$this->data['modified'] = '';
		}
		...
		if (isset($this->request->post['available'])) {
			...
			$this->data['available'] = date('Y-m-d H:i', strtotime($article_info['available']));
		} else {
			$this->data['available'] = date('Y-m-d H:i', time() - 64800);
		}
		...
		if (isset($this->request->post['robots'])) {
			...
			$this->data['robots'] = $article_info['robots'];
		} else {
			$this->data['robots'] = '';
		}
	...
	public function catproduct() {
		...
		$product_info = array();
		$results = $this->model_blog_article->getProductsByCategoryId($category_id);
