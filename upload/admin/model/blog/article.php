...
class ModelBlogArticle extends Model {
	public function addArticle($data, $date) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "blog_article SET author_id = '" . (int)$data['author_id'] . "', image = '" . $this->db->escape($data['image']) . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', comment = '" . (int)$data['comment'] . "', robots = '" . $this->db->escape($data['robots']) . "', available = '" . $this->db->escape($data['available']) . "', created = '" . $this->db->escape($data['created']) . "', modified = '" . $this->db->escape($date) . "'");
	...
	public function editArticle($article_id, $data, $date) {
		$this->db->query("UPDATE " . DB_PREFIX . "blog_article SET author_id = '" . (int)$data['author_id'] . "', image = '" . $this->db->escape($data['image']) . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', comment = '" . (int)$data['comment'] . "', robots = '" . $this->db->escape($data['robots']) . "', available = '" . $this->db->escape($data['available']) . "', created = '" . $this->db->escape($data['created']) . "' , modified = '" . $this->db->escape($date) . "' WHERE article_id = '" . (int)$article_id . "'");
	...
	public function getArticles($data = array()) {
		if ($data) {
			...
			if ((!empty($data['filter_category']) && $data['filter_category'] !='non') || $data['sort'] == 'a2c.category_id') {
				$sql .= " LEFT JOIN " . DB_PREFIX . "blog_article_to_category a2c ON (a.article_id = a2c.article_id)";			
			}
         
         if ($data['filter_store'] != 'all' || $data['sort'] == 'a2s.store_id') {
         ...
         $sql .= " WHERE ad.language_id = '" . (int)$this->config->get('config_language_id') . "'";
			...
			if (!empty($data['filter_category']) && $data['filter_category'] !='non') {
         ...
         if (!empty($data['filter_category']) && $data['filter_category'] =='non') {
            $sql .= " AND a.article_id NOT IN (SELECT a2c.article_id FROM " . DB_PREFIX . "blog_article_to_category a2c)";
			}
         ...
         if (isset($data['sort']) && $data['sort'] == 'a2c.category_id') {
				$sql .= " AND a.article_id = a2c.article_id";
			}
			
			if (isset($data['sort']) && $data['sort'] == 'a2s.store_id') {
				$sql .= " AND a.article_id = a2s.article_id";
			}
	...
	public function getArticlesByCategoryId($category_id) {
      ...
      if (!empty($category_id) && ($category_id !='all') && ($category_id !='non')) {
         $sql .= " LEFT JOIN " . DB_PREFIX . "blog_article_to_category a2c ON (a.article_id = a2c.article_id)";			
      }
      
      $sql .= " WHERE ad.language_id = '" . (int)((method_exists($this->language, 'getId')) ? (int)$this->language->getId() : (int)$this->config->get('config_language_id')) . "'";
      
      if (!empty($category_id) && ($category_id !='all') && ($category_id !='non')) {
         $sql .= " AND a2c.category_id = '" . (int)$category_id . "'";
      }
