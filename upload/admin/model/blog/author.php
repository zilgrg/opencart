...
class ModelBlogAuthor extends Model {
   ...
   public function changeAuthor($data) {
      $this->db->query("UPDATE " . DB_PREFIX . "blog_article SET author_id = '" . (int)$data['changeAuthor'] . "' WHERE author_id = '" . (int)$data['from_author_id'] . "'");
   }
   
   public function getAuthors() {
      $query = $this->db->query("SELECT a.author_id, a.user_id, a.name, CONCAT(u.firstname, ' ', u.lastname) AS fullname, u.user_id AS oc_user_id, (SELECT name FROM " . DB_PREFIX . "blog_author_group ag WHERE ag.author_group_id = a.author_group_id) AS groupname FROM " . DB_PREFIX . "blog_author a LEFT JOIN " . DB_PREFIX . "user u ON (a.user_id = u.user_id)");
   ...
   public function getUsers() {
      ...
      if ($authors) { 
         ...
         $except = " WHERE " . implode(' AND ', $implode_data) . " AND status = '1' "; 
      } else { 
   ...
   public function checkAdminUser() {
      $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "blog_author ba JOIN " . DB_PREFIX . "user u ON (ba.user_id = u.user_id) WHERE ba.author_group_id = '1'");
