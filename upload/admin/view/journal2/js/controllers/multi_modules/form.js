define(['./../module', 'underscore'], function (module, _) {
    ...
    module.factory('MultiModulesFactory', [function () {
        return {
            Column: function () {
                return {
                    is_open: true,
    ...
    module.controller('MultiModulesFormController', function ($scope, $routeParams, $location, Rest, Spinner, MultiModulesFactory) {
        /* opened modules */
        $scope.module_id = $routeParams.module_id || null;

        /* scope vars */
        $scope.module_type = 'multi_modules';
        ...
        Rest.all(promises, function (response) {
            if ($scope.module_id) {
                ...
                $scope.module_data.columns = _.map($scope.module_data.columns, function (column) {
                    var columns = _.extend(new MultiModulesFactory.Column(), column);
                    columns.modules = _.map(columns.modules, function (module) {
                        return _.extend(new MultiModulesFactory.Module(), module);
                    });
                    return columns;
                });
            }
        ...
        $scope.addColumn = function () {
            $scope.module_data.columns.push(new MultiModulesFactory.Column());
        };
        ...
        $scope.addModule = function (column) {
            column.modules.push(new MultiModulesFactory.Module());
        };
