...
var categoryTemplate = {
	...
	patternLine : {
		init : function($input){
			$input.popover({html : true, placement : 'top', trigger : 'manual'});
			
			$input.focus(function(e){
				
				categoryTemplate.patternLine.show($(this));
			...
			$(document).click(function() {
				 categoryTemplate.patternLine.hide($(this));
			});
		},
		show: function($input){
			...
			var $popover = $input.closest('.modal-categoryTemlate').find('.popover');
			
			var html = $input.closest('.modal-categoryTemlate').find('.pattern_line_label_hide').html();
			
			$popover.find('.popover-content').html(html);
			
			$popover.find('.close-popup').click(function(){
				categoryTemplate.patternLine.hide($input);
			});

			//$popover.find('*[data-toggle=tooltip]').tooltip({html:true});

			//categoryTemplate.patternAddValue.init($popover);
		},
		hide: function($input){
			$input.popover('hide');
		}
	...
	patternAddValue : {
		...
		init : function($modal){
			...
			$input = $modal.closest('.modal-categoryTemlate').find('.seo_input_pattern');
			
			categoryTemplate.patternAddValue.data.actInput = $input;
