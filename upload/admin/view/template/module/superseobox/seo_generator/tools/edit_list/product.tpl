<div class="pull-right input-append">
	...
	<a data-type="product" class="btn seo-edit-list-search" type="button">
		Search
	</a>
	<a data-href="<?php echo $urls['ajax'].'&metaData[action]=getGenEditorContent&data[type]=product'; ?>" class="btn seo-edit-list-all" type="button">
		Show all
	</a>
</div>
...
<div class="accordion seo_edit-list" id="seo_edit-list-product">
<?php $i_tr = 1; foreach ($content['content'] as $item_id => $cont_data) { ?> 
	<div class="accordion-group">
		 <input type="hidden" name="data[item_type]" style="width:90%;" type="text" value="product">
		 <input type="hidden" name="data[item_id]" style="width:90%;" type="text" value="<?php echo $item_id; ?>">
		<div class="accordion-heading">
			<a class="accordion-toggle" data-toggle="collapse" data-parent="#seo_edit-list-product" href="#<?php echo 'collapse-product-'.$i_tr.'-'.$item_id; ?>">
				<span>
		...
		<div id="<?php echo 'collapse-product-'.$i_tr.'-'.$item_id; ?>" class="accordion-body collapse">
			<div class="accordion-inner">
				<ul class="nav nav-tabs" id="">
				<?php $i_tb = 1; foreach ($cont_data['data'] as $lang_id => $data) { ?>
					<li class="<?php if($i_tb ==1){echo 'active';} ?>">
						<a href="#<?php echo 'seo_edit-product-'.$i_tb.'-'.$lang_id.'-'.$item_id; ?>" data-toggle="tab">
				...
				<?php $i_tp = 1; foreach ($cont_data['data'] as $lang_id => $data) { ?>	
					<div class="tab-pane <?php if($i_tp ==1){echo 'active';} ?>" id="<?php echo 'seo_edit-product-'.$i_tp.'-'.$lang_id.'-'.$item_id; ?>">
				...
				<div class="control-group">
					...
					<div class="seo-edit-related">
						<?php $i_rel = 0; foreach ($content['content'][$item_id]['related'] as $data_r) { ?>
						<div class="pull-left">	
							<a class="btn "><?php echo $data_r['name']; ?> X</a>
							<input type="hidden" name="data[related_product][<?php echo $i_rel; ?>]" style="background-color: #fff;" class="add-on" type="text" value="<?php echo $data_r['related_id']; ?>">
						</div>
						<?php $i_rel++; } ?>
					</div>
				</div>
